...
class MultimodalWorkerHandler(BaseWorkerHandler):
    ...
    async def _generate_aggregated(
        ...
        try:
            ...
            # Create multimodal items
            mm_items = []
            for embeddings, image_grid_thw in zip(
                embeddings_list, image_grid_thw_list
            ):
                mm_items.append(
                    self.embeddings_processor.create_multimodal_item(
                        embeddings, image_grid_thw
                    )
                )
                logger.debug(
...
class MultimodalPrefillWorkerHandler(BaseWorkerHandler):
    ...
    async def _process_prefill_generation(
        ...
        # Create multimodal items for prefill generation
        mm_items = []
        for embeddings, image_grid_thw in zip(
            embeddings_list, image_grid_thw_list
        ):
